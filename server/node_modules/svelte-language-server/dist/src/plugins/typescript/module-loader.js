...
class ImpliedNodeFormatResolver {
    ...
    cacheImpliedNodeFormat(sourceFile, compilerOptions) {
        if (!sourceFile.impliedNodeFormat && (0, utils_2.isSvelteFilePath)(sourceFile.fileName)) {
            ...
            if (!this.alreadyResolved.has(sourceFile.fileName)) {
                sourceFile.impliedNodeFormat = typescript_1.default.getImpliedNodeFormatForFile((0, utils_2.toVirtualSvelteFilePath)(sourceFile.fileName), undefined, this.tsSystem, compilerOptions);
    ...
    resolveForTypeReference(entry, sourceFile, compilerOptions) {
        let mode = undefined;
        if (sourceFile) {
            this.cacheImpliedNodeFormat(sourceFile, compilerOptions);
...
function createSvelteModuleLoader(getSnapshot, compilerOptions, tsSystem, tsModule, getModuleResolutionHost) {
    ...
    const failedPathToContainingFile = new fileCollection_1.FileMap();
    const failedLocationInvalidated = new fileCollection_1.FileSet();
    return {
    ...
    function resolveModuleNames(moduleNames, containingFile, _reusedNames, redirectedReference, options, containingSourceFile) {
        return moduleNames.map((moduleName, index) => {
            ...
            resolvedModule?.failedLookupLocations?.forEach((failedLocation) => {
                const failedPaths = failedPathToContainingFile.get(failedLocation) ?? new fileCollection_1.FileSet();
    ...
    function resolveTypeReferenceDirectiveReferences(typeDirectiveNames, containingFile, redirectedReference, options, containingSourceFile) {
        return typeDirectiveNames.map((typeDirectiveName) => {
            ...
            const mode = impliedNodeFormatResolver.resolveForTypeReference(entry, containingSourceFile, options);
